...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        for level, mult in enumerate(channel_mults):
            ...
            if level != len(channel_mults) - 1:
                ...
                ds *= 2
                self._feature_size += ch
                if ds in in_feasture_res:
                    self.input_blocks[-1].ins_after=in_feasture_res.index(ds)
        ...
        for level, mult in list(enumerate(channel_mults))[::-1]:
            for i in range(res_blocks + 1):
                ...
                if level and i == res_blocks:
                    ...
                    if ds in in_feasture_res:
                        layers[-1].ins_before=2*len(in_feasture_res)-1-in_feasture_res.index(ds)
                    ds //= 2
    ...
    def forward(self, x,image_features,self_cond_prev, gammas):
        """
        ...
        if self_cond_prev is not None:
            x=torch.cat([x,self_cond_prev],dim=1)
        h = x.type(torch.float32)
        for module in self.input_blocks:
            h = module(h, emb)
            if getattr(module,'ins_after',None) is not None:
                h=self.feature_inject(h,image_features[module.ins_after ])
            hs.append(h)
        h = self.middle_block(h, emb)
        for module in self.output_blocks:
            if getattr(module,'ins_before',None) is not None:
                h=self.feature_inject(h,image_features[module.ins_before ])
