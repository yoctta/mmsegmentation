...
class GaussianDiffusionSeg(ABC):
    ...
    def train_loss(
            ...
            t=None,
            **kwargs):
        terms = self.training_losses(self.class1hot(batch['seg']),t=t,model_kwargs=dict(image=batch['image']))
        out = {}
        valid_mask=batch['seg']!=self.ignore_class # B H W 
        sum_valid_mask=torch.sum(valid_mask)
        out['loss'] = torch.sum(terms['loss']*valid_mask)/sum_valid_mask
        out['acc_seg']=torch.sum((torch.argmax(terms['logits'],dim=1)==batch['seg']) & valid_mask)/sum_valid_mask
        return out
